...
global.getEmotionContents = (content) => {
  ...
  for(var i=0;i<cs.length;i++){
    ...
    if(acs.length > 10 || !emtioncs[acs]){
        es.push(<React.Text key={i}>{acs}</React.Text>) ;
        continue ;
    }
    es.push(<React.Image key={i} source={{lineHeight:30,width:15,height:15,uri:'http://img.lmjx.net/i/jiyouhui/emotion/em/'+ emtioncs[acs] +'.gif'}} />) ;
  }
